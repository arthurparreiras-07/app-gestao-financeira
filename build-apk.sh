...
if [ ! -d "assets" ]; then
    echo "⚠️  Pasta assets não encontrada. Criando placeholders..."
    mkdir -p assets
    
    # Criar placeholders usando placeholder.com
    if command -v wget &> /dev/null; then
        echo "   Baixando placeholders temporários..."
        wget -q "https://via.placeholder.com/1024x1024/4F46E5/FFFFFF?text=MindBudget" -O assets/icon.png
        wget -q "https://via.placeholder.com/1024x1024/4F46E5/FFFFFF?text=MB" -O assets/adaptive-icon.png
        wget -q "https://via.placeholder.com/1284x2778/4F46E5/FFFFFF?text=MindBudget" -O assets/splash.png
        echo "   ✅ Placeholders criados em assets/"
    else
        echo "   ⚠️  wget não encontrado. Crie os assets manualmente:"
