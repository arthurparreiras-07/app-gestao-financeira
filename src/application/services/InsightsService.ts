...
generateInsights(
  ...
  Object.entries(savingsByEmotion).forEach(([emotionId, emotionSavings]) => {
    ...
    if (avgAmount > averageSaving * 1.2) {
      const percentage = Math.round(((avgAmount - averageSaving) / averageSaving) * 100);
      insights.push({
        id: `saving-emotion-${emotionId}`,
        type: 'success',
        message: `Você economiza ${percentage}% mais quando está ${emotion.name.toLowerCase()} ${emotion.icon}`,
        icon: '💰',
      });
    }
  });
  ...
  if (recentSavings.length >= 5) {
    ...
    insights.push({
      id: 'recent-savings',
      type: 'success',
      message: `Você economizou R$ ${recentTotal.toFixed(2)} nos últimos ${recentSavings.length} registros`,
      icon: '💵',
    });
  }
  ...
  if (allExpenses.length > 0 && allSavings.length > 0) {
    ...
    if (totalSavings > totalExpenses) {
      const percentage = Math.round(((totalSavings - totalExpenses) / totalExpenses) * 100);
      insights.push({
      ...
      const percentage = Math.round(((totalExpenses - totalSavings) / totalSavings) * 100);
      insights.push({
  ...
  if (allSavings.length >= 3) {
    ...
    const monthlySavings = allSavings.filter(
      (s) => s.date.getMonth() === thisMonth && s.date.getFullYear() === thisYear
    );
